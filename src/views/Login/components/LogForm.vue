...
async function onSubmit() {
  ...
  ruleFormRef.value?.validate(async (valid) => {
    if (valid) {
      ...
      if (res?.code === 200) {
        localStorage.setItem('hoo_guild_remember', remember.value + '')
        const basePassword = Base64.encode(form.password) // 加密
        if (remember.value) {
          localStorage.setItem('hoo_guild_username', form.username)
          localStorage.setItem('hoo_guild_password', basePassword)
        } else {
          localStorage.removeItem('hoo_guild_username')
          localStorage.removeItem('hoo_guild_password')
        }
...
onMounted(() => {
  const is_remember = localStorage.getItem('hoo_guild_remember')
  if (is_remember === 'true') {
    remember.value = true
  } else {
    remember.value = false
  }
  const username = localStorage.getItem('hoo_guild_username')
  const password = localStorage.getItem('hoo_guild_password')
